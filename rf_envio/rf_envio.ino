...
int senhaNormal; // Variável que indica para o receiver que é senha normal
int senhaPref; // Variável que indica para o receiver que é senha preferencial
...
void loop()
{
  ...
  if (digitalRead(botao1) == HIGH && digitalRead(botao2) == LOW)
  {
    ...
    if ((unsigned long)(currentMillis_botao1 - previousMillis_botao1) >= intervalo)
    {
      ...
      senhaNormal = cont_senha; // Variável que recebe a mensagem a ser transmitida
      radio.write(&senhaNormal, sizeof(senhaNormal)); // Envia a mensagem ao receptor
  ...
  if (digitalRead(botao2) == HIGH && digitalRead(botao1) == LOW)
  {
    ...
    if ((unsigned long)(currentMillis_botao2 - previousMillis_botao2) >= intervalo)
    {
      ...
      senhaPref = cont_senha_pref;
      radio.write(&senhaPref, sizeof(senhaPref)); // Envia a mensagem ao receptor

      cont_senha_pref++;

      if (cont_senha_pref == 1001)
      {
        cont_senha_pref = 501;
      }
